            ...
            <div class="hidden md:flex items-center gap-3">
                <?php if ($user): ?>
                    <div class="flex items-center gap-2 pl-2 border-l border-zinc-200">
                        <div class="w-8 h-8 rounded-full bg-zinc-900 text-white flex items-center justify-center text-xs font-bold overflow-hidden">
                            <?php if (!empty($user['avatar_url'])): ?>
                                <img src="<?= htmlspecialchars($user['avatar_url']) ?>" alt="" class="w-full h-full object-cover">
                            <?php else: ?>
            ...
            <div class="px-4 pb-4 border-t border-zinc-100 pt-3">
                <?php if ($user): ?>
                    <div class="flex items-center gap-3">
                        <div class="w-9 h-9 rounded-full bg-zinc-900 text-white flex items-center justify-center text-xs font-bold overflow-hidden">
                            <?php if (!empty($user['avatar_url'])): ?>
                                <img src="<?= htmlspecialchars($user['avatar_url']) ?>" alt="" class="w-full h-full object-cover">
                            <?php else: ?>
                                <?= strtoupper(substr($user['name'] ?? 'U', 0, 1)) ?>
